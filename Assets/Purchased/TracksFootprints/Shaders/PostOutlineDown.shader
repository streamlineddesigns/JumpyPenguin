Shader "Custom/PostOutlineDown" {
	Properties
    {
        _MainTex("Main Texture",2D)="white"{}
		_OutlineIntensity("Outline Intensity", int)=1
    }
    SubShader
    {
    Blend SrcAlpha OneMinusSrcAlpha
        Pass
        {
            ...
            half4 frag(v2f i) : COLOR
            {
                ...
                int _NumberOfIterations = _OutlineIntensity;
                ...
                for(int k=0;k<_NumberOfIterations;k+=1)
                {
                    ...
                    for(int j=0;j<_NumberOfIterations;j+=1)
                    {
                        ...
                        ColorIntensityInRadius+=tex2D( _MainTex, i.uvs.xy+float2((k-_NumberOfIterations/2)*TX_x,(j-_NumberOfIterations/2)*TX_y)).r;
                    }
                }
